...
@section('title', __('messages.adm_modify_user') . ($user->name ?? ''))
@section('page-title', __('messages.adm_modify_user') . ($user->name ?? ''))
...
<style>
    ...
    .role-name  { font-size: 12px; font-weight: 700; color: var(--text, #111827); }
    .role-desc  { font-size: 11px; color: #9ca3af; line-height: 1.4; }
    ...
    .meta-key { color: #9ca3af; font-weight: 500; }

    .meta-val { color: var(--text, #111827); font-weight: 600; }
...
<div class="page-header">
    <div>
        ...
        <h1>{{ __('messages.adm_modify_user') }}{{ $user->name }}</h1>
        <p>{{ __('messages.adm_update_account') }} · #{{ $user->id }}</p>
    </div>
        ...
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST" id="userForm">
                ...
                <div class="form-grid-2">
                    <div class="form-group full">
                        ...
                        <input id="nameInput" type="text" name="name"
                               ...
                               placeholder="{{ __('messages.adm_full_name') }}" required>
                ...
                <div class="form-grid-2">
                    <div class="form-group">
                        <label class="form-label">
                            {{ __('messages.adm_new_password') }}
                            <span class="optional-tag">{{ __('messages.adm_optional') }}</span>
                        </label>
                        <div class="pw-wrap">
                            <input id="pwField" type="password" name="password"
                                   class="form-control @error('password') is-invalid @enderror"
                                   placeholder="{{ __('messages.adm_leave_empty') }}">
                        ...
                        <span class="form-hint">{{ __('messages.adm_leave_empty') }}</span>
                        @error('password')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">
                            {{ __('messages.adm_confirm_password') }}
                            <span class="optional-tag">{{ __('messages.adm_optional') }}</span>
                        </label>
                        <div class="pw-wrap">
                    ...
                    @foreach([
                        ...
                        <div>
                            <input type="radio" name="role" id="role_{{ $role['value'] }}"
                                   ...
                                   {{ old('role', $user->role) === $role['value'] ? 'checked' : '' }}
                                   onchange="updateRoleBadge('{{ $role['value'] }}')">
            ...
            <div class="form-footer">
                ...
                <button type="submit" class="btn-submit">
                    <svg viewBox="0 0 24 24" fill="none">
                        <path d="M19 21H5a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v14a2 2 0 01-2 2z" stroke-linecap="round" stroke-linejoin="round"/>
    ...
    <div>
        <div class="sidebar-card">
            <div class="preview-avatar" id="previewAvatar">
                {{ mb_strtoupper(mb_substr($user->name, 0, 1)) }}{{ mb_strtoupper(mb_substr(strstr($user->name, ' '), 1, 1)) }}
            </div>
            <div class="preview-name" id="previewName">{{ $user->name }}</div>
            <div class="preview-email" id="previewEmail">{{ $user->email }}</div>

            @php
                $roleClasses = ['client'=>'role-client','employee'=>'role-employee','admin'=>'role-admin'];
                $roleLabels  = ['client'=>'Cliente','employee'=>'Employée','admin'=>'Admin'];
            @endphp
            <span class="preview-role-badge {{ $roleClasses[$user->role] ?? 'role-client' }}"
                  id="previewRole">
            ...
            <div class="sidebar-meta">
                <div class="meta-row">
                    ...
                    <span class="meta-val">#{{ $user->id }}</span>
                </div>
                <div class="meta-row">
                    <span class="meta-key">{{ __('messages.adm_registered_on') }}</span>
                    <span class="meta-val">{{ $user->created_at->format('d/m/Y') }}</span>
                </div>
                <div class="meta-row">
                    <span class="meta-key">{{ __('messages.adm_last_update') }}</span>
                    <span class="meta-val">{{ $user->updated_at->format('d/m/Y') }}</span>
                </div>
            </div>
...
<script>
    ...
    nameInput?.addEventListener('input', function () {
        ...
        if (val) {
            ...
            previewAvatar.textContent = parts.map(w => w[0]).slice(0, 2).join('').toUpperCase();
        } else {
    ...
    if (checked) updateRoleBadge(checked.value);
</script>
